...
def check_args(args):
    ...
    Returns:
        List of arguments if valid, else prints an error message.
    """
    arg_list = parse(args)

    if len(arg_list) == 0:
        print("** class name missing **")
    elif arg_list[0] not in SUPPORTED_CLASSES:
...
class HBNBCommand(cmd.Cmd):
    ...
    def emptyline(self):
        """
        Command to be executed when an empty line + <ENTER> key.
        """
        pass
    ...
    def do_all(self, argv):
        ...
        if not arg_list:
            ...
            if arg_list[0] not in SUPPORTED_CLASSES:
                print("** class doesn't exist **")
            else:
                print([str(obj) for obj in objects
